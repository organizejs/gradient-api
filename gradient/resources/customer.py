...
from ..models import (
  UserModel, CustomerModel, CustomerSchema, 
  MaritalStatus
)
...
class CustomerRegister(Resource):
  ...
  registration_args = {
    ...
    "marital_status": fields.Str(
      required=True,
      validate=validate.OneOf([e.value for e in MaritalStatus])
    )
  }
  ...
  def post(self, args):
    ...
    if current_user.customer is not None:
      return {
        "message": "This user {} is already registered.".format(current_user.email)
      }, 404
